...
int songoffset[MAX_SONGS][MAX_PLAY_CH];	// Changed to allow for 9 channel SID 
int songsize[MAX_SONGS][MAX_PLAY_CH];
int tableerror;
int channels;
int fixedparams;
int simplepulse;
...
void relocator(GTOBJECT *gt)
{
	...
	for (c = 0; c < songs; c++)
	{
		...
		for (d = 0; d < MAX_CHN; d++)		// MAX_CHN JP
		{
			if (maxSIDChannels == 9 && oddEvenSubSong == 1 && d >= 3)
				break;
			if (maxSIDChannels == 3 && d >= 3)
