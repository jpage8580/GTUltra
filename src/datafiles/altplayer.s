...
SETCUTOFF       = $00
	...
	; JP MODIFIED - A=0-$fe: Requires X and Y to contain Hi/Lo address of other .SID file first channel tempo
	; A=$ff: X and Y returned, containing this .SID Hi/Lo address of first channel tempo
                jmp mt_init	
                jmp mt_play
...
; Hand shake between the two SID files within this single .SID file
	...
	lda #$ff
	jsr $4000	; << get tempo position from other SID (calls other SID init routine)
	lda #$fe
	jsr mt_actual_init	; Set tempo position for this SID
	lda #$ff
	jsr mt_actual_init	; get tempo position for this SID
	lda #$fe
	jsr $4000	; << set tempo position from other SID

.jpta ldy #0	; <<< self mod (actual song number)
	iny
	tya
	jsr $4000	; init other song
